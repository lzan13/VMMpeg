...
int VMVideoEncoder::encodeFrame(uint8_t *inBytes, int frameSize, int pts, uint8_t *outBytes, int *outFrameSize) {
    //YUV420P数据转化为h264
 ...
 * 初始化x264配置
 */
void VMVideoEncoder::initX264Params() {
    // 默认：medium
    ...
    /**
     * X264 编码器预设值 preset
     * 一些在压缩效率和运算时间中平衡的预设值。如果指定了一个预设值，它会在其它选项生效前生效，预设按照编码速度降序排列为:
     * ultrafast, superfast, veryfast, faster, fast, medium, slow, slower, veryslow, placebo
     * 默认：medium
     * 可以使用 --preset 来查看预设列表，也可以通过 x264 --fullhelp 来查看预设所采用的参数配置
     *
     * tune 这个参数是在上一个选项基础上进一步优化输入，如果定义了一个tune值，它将在preset之后，其它选项之前生效
     * 可选值：film, animation, grain, stillimage, psnr, ssim, fastdecode, zerolatency, touhou.
     *
     */
    x264_param_default_preset(&params, "superfast", "grain");

    //I帧间隔
    params.i_csp = X264_CSP_I420;
    params.i_width = getWidth();
    params.i_height = getHeight();
...
int VMVideoEncoder::getFps() const {
    return fps;
}

void VMVideoEncoder::setFps(int fps) {
    this->fps = fps;
}

int VMVideoEncoder::getNumNals() const {
    return num_nals;
}

void VMVideoEncoder::setNumNals(int numNals) {
    num_nals = numNals;
}

int VMVideoEncoder::getHeight() const {
    return height;
}

void VMVideoEncoder::setHeight(int outHeight) {
    height = outHeight;
}

int VMVideoEncoder::getWidth() const {
    return width;
}

void VMVideoEncoder::setWidth(int outWidth) {
    width = outWidth;
}

int VMVideoEncoder::getBitrate() const {
    return bitrate;
}

void VMVideoEncoder::setBitrate(int bitrate) {
    this->bitrate = bitrate;
}

int VMVideoEncoder::getSliceMaxSize() const {
    return i_slice_max_size;
}

void VMVideoEncoder::setSliceMaxSize(int sliceMaxSize) {
    i_slice_max_size = sliceMaxSize;
}

int VMVideoEncoder::getVbvBufferSize() const {
    return i_vbv_buffer_size;
}

void VMVideoEncoder::setVbvBufferSize(int vbvBufferSize) {
    i_vbv_buffer_size = vbvBufferSize;
}

int VMVideoEncoder::getIThreads() const {
    return i_threads;
}

void VMVideoEncoder::setIThreads(int threads) {
    i_threads = threads;
}

int VMVideoEncoder::getBFrameFrq() const {
    return b_frame_frq;
}

void VMVideoEncoder::setBFrameFrq(int frameFrq) {
